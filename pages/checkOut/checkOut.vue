<template>
	<view class="container">
		...
		<view class='add_img_card card'>
			<view class='title'>添加检查/返工图片</view>
			<view class='img_list'>
		...
		<view class='footer'>
			<button @tap='is_submit'>提交</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		onLoad() {
			...
			request('/api/auth/getOrder', 'POST', {
				...
				this.shoeboxImgArr = res.data[0].back_image === null ? [] : JSON.parse(res.data[0].back_image);
				this.shoeboxImgResArr = res.data[0].back_image === null ? [] : JSON.parse(res.data[0].back_image);
				this.sound = res.data[0].sound;
				this.time = res.data[0].sound_length;
			...
			is_submit() {
				...
				if (that.voice !== "") {
					that.upRecord(that.voice, that.qiniuToken).then(res => {
						sound = res.imageURL;
						console.log(sound);
						that.upImgs(this.shoeboxImgArr, that.qiniuToken).then(back_image => {
							back_image = [...new Set(back_image.concat(...this.shoeboxImgResArr))];
							console.log(back_image);
							request("/api/auth/updateOrder", "POST", {
								wash_id: that.jsId,
								bar_id: this.shapecode,
								back_image,
